...
@section('title', 'Surat Keterangan KPR')

@section('content')
<meta name="csrf-token" content="{{ csrf_token() }}" />
<meta name="_base_url" content="{{ url('/') }}">

<form id="frm_tambah_suket_kprbank" name="frm_tambah_suket_kprbank">
    <div class="card-body">
        <div class="row">
            <table class="table table-sm">
                <tr>
                    ...
                    <td><input type="text" id="nosurat" name="nosurat" style="width: 5%;"> / <input type="text"
                            value="{{ $alias_surat }}/" style="border: none;outline: none;width: 8%;"><select
                            id="bulan" name="bulan">
                ...
                <tr>
                    <td>Nama Bank KPR</td>
                    <td>:</td>
                    <td><select id="namabank" name="namabank" class="form-control select2"
                            style="width: 45%;">>
                            <option selected="selected">-- Pilih Bank KPR --</option>
                            @foreach ($nama_bank as $nama_banks)
                            <option value="{{ $nama_banks->ID }}">{{ $nama_banks->NamaBank }}
                            </option>
                            @endforeach
                        </select></td>
                </tr>
...
<diV class="w3-center" id="simpan">
    <input type="button" id="btn_simpan_data" class="w3-btn w3-round-medium w3-indigo w3-large" value="Simpan"
        onclick="fn_tambah_suket_kpr()" />

    <a href="{{ url('/kpr') }}" class="w3-btn w3-round-small w3-red w3-large">Kembali</a>
</diV>
...
<script>
    ...
    function fn_tambah_suket_kpr() {
        ...
        } else if (namabank == "-- Pilih Bank KPR --") {
            swal({
                title: "error!",
                text: "Bank KPR masih kosong",
                type: "error",
                timer: 2000,
                confirmButtonText: "Tutup",
            });
